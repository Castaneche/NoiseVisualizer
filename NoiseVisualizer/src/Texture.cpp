...
void Texture::Init(float imagesize, int pixelcount, std::shared_ptr<SetupData> setupdata)
{
	pixels = nullptr;
	...
	imageSize = imagesize;
	this->resizeImg(pixelcount); //generate array of pixels
...
void Texture::ShowTexture()
{
	...
	{
		std::lock_guard<std::mutex> guard(mutex);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, pixelCount, pixelCount, 0, GL_RGB, GL_UNSIGNED_BYTE, this->pixels);
	}
	glBindTexture(GL_TEXTURE_2D, 0);
